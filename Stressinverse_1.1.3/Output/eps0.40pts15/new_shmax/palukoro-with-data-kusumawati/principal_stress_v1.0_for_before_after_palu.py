...
cluster_folder=['datawithkusumawati(A-B)before',
                'datawithkusumawati(A-B)after',
                'datawithkusumawati(A-D)before',
                'datawithkusumawati(A-D)after'
                ]
lonlat_folder=['/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/palukoro-with-data-kusumawati/datawithkusumawati(A-B)before.csv', 
               '/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/palukoro-with-data-kusumawati/datawithkusumawati(A-B)after.csv',
               '/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/palukoro-with-data-kusumawati/datawithkusumawati(A-D)before.csv', 
               '/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/palukoro-with-data-kusumawati/datawithkusumawati(A-D)after.csv'
               ]
parameter = [0, 1]
...
i=0
for file in range(len(cluster_folder)):
    #for clus in range(10):
        data = pd.read_csv('/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/palukoro-with-data-kusumawati/'+cluster_folder[file]+"/output_origin.csv")
        data_bootstrap = pd.read_csv('/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/palukoro-with-data-kusumawati/'+cluster_folder[file]+"/output_bootstarap.csv")
        data_cluster = pd.read_csv(lonlat_folder[file])
        ...
        #matrix[i][0] = parameter[file]; matrix[i][1] = float(0); 
        matrix[i][2] = mean_longitude; matrix[i][3] = mean_latitude 
...
table = pd.DataFrame(matrix, columns=["name", "quality", "lon", "lat", "depth", "strike1", "dip1", "rake1", "strike2", "dip2", "rake2", "Mw", "down", "up", "R", "type", "K", "AR"])
...
nama = ['before', 'after', 'before', 'after']
quality = ['A-B', 'A-B', 'A-D', 'A-D']
table['name'] = nama
...
table.to_csv("/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/palukoro-with-data-kusumawati/shmax.csv", index=False)