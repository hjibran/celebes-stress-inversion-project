...
tablea = pd.read_csv("/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/shmax/shmax_wilayah_laut_maluku.csv")

table1 = tablea#table.query('name == 15.40')
table1 = table1.reset_index()

FM = table1.iloc[:, [2, 3, 4, 5, 6, 7, 11]]
FM.columns = ["longitude", "latitude", "depth", "strike", "dip", "rake", "magnitude"]

tableb = pd.read_csv("/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/shmax/shmax_wilayah_barat_sangihe.csv")

table2 = tableb#table.query('name == 15.40')
table2 = table2.reset_index()

FM = table2.iloc[:, [2, 3, 4, 5, 6, 7, 11]]
FM.columns = ["longitude", "latitude", "depth", "strike", "dip", "rake", "magnitude"]
...
fig.coast(region=[123.3, 129.5, -2.05, 6.3],
          projection='M10c',
          frame=["a1", "EWSN"],
          land='lightgray',
          resolution='f',
...
for i in range(len(table2)):
    shmax_plot(table2['lon'][i], table2['lat'][i], table2['down'][i], table2['up'][i], table1['AR'][i])
