...
# cluster 0.35
cluster_boundary = gpd.read_file("/mnt/d/celebes-stress-inversion-project/Result/eps0.35min15-final/eps0.35pts15.shp")
fig.plot(data=cluster_boundary, pen="1p,green", label="ε=0.35 MinPts=15")
...
def shmax_plot(lon, lat, down, up):
    ...
    fig.plot(data=data, style="w", fill="green", transparency=50)#, pen="1p, red")
...
def shmax_plot(lon, lat, down, up):
    ...
    data = [[lon, lat, 1.5, 360-up+90, 360-down+90], [lon, lat, 1.5, 360-up1+90, 360-down1+90]]
    fig.plot(data=data, style="w", fill="blue", transparency=50)#, pen="1p, red")

for i in range(10):
    data_cluster = pd.read_csv("/mnt/d/celebes-stress-inversion-project/Result/eps0.40min15-final/clustered/cls{}.csv".format(i))
    lon = data_cluster["lon"];mean_longitude = np.mean(lon)
    lat = data_cluster["lat"];mean_latitude = np.mean(lat)
    data_shmax = pd.read_csv("/mnt/d/celebes-stress-inversion-project/Stressinverse_1.1.3/Output/eps0.40pts15/new_shmax/cls{}/output_error.csv".format(i))
    min_shmax = data_shmax["2.5%"][6]; max_shmax = data_shmax["97.5%"][6]
...
fig.plot(data=cluster_boundary, pen="1p,red", label="ε=0.50 MinPts=15")
...
def shmax_plot(lon, lat, down, up):
    ...
    data = [[lon, lat, 1.5, 360-up+90, 360-down+90], [lon, lat, 1.5, 360-up1+90, 360-down1+90]]
    fig.plot(data=data, style="w", fill="red", transparency=50)#, pen="1p, red")
