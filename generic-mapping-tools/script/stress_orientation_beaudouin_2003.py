...
grid = pygmt.datasets.load_earth_relief(resolution="30s", # resolution of earth relief
                                        region=[118, 127, -4.5, 3], #[117.5, 128, -9, 6] boarder of map: [minlon, maxlon, minlat, maxlat]
...
fig.grdimage(grid=grid, # call grid
             frame=["a", "EWSN"],
             projection="M30c",
             cmap= 'oleron' #'grayC',#"/mnt/d/celebes-stress-inversion-project/generic-mapping-tools/script/color.cpt"
)
...
struktur(2.5, 'dimgray')

region = gpd.read_file("/mnt/d/celebes-stress-inversion-project/data/Beaudouin2003/region_beadouin.shp")
fig.plot(data=region, pen="3.5p,black,-", label="microblock (Soquet, 2006)")
...
fig.basemap(rose="jBR+3c+lW,E,S,N+o0.3c/0.3c+f2")
