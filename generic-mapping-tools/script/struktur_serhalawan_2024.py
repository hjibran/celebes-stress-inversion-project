...
grid = pygmt.datasets.load_earth_relief(resolution="30s", # resolution of earth relief
                                        region=[117.9, 128, -7.2, 3.5], #[117.5, 128, -9, 6] boarder of map: [minlon, maxlon, minlat, maxlat]
                                        registration="gridline")

pygmt.makecpt(cmap='terra', series=f"{-12000}/{12000}/{1}")

fig.grdimage(grid=grid, # call grid
             frame=["a", "EWSN"],
             projection="M30c",
             cmap= True,
             shading = True #'grayC',#"/mnt/d/celebes-stress-inversion-project/generic-mapping-tools/script/color.cpt"
)
